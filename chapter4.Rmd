...
Now we will look at clusterin, and we will start with distance measures. First, we need to reload the Boston dataset and standardize the dataset.
...
But now the question arises: what is the optimal number of clusters? In other words, we need to determine the K. There are more than one way to skin the proverbial cat here, but we will look at how the total of *within cluster sum of squares* or WCSS behaves when we change the number of clusters. We will achieve this by plotting the number of clusters and the total WCSS. Once the WCSS drops radically, we have the optimal number of clusters. So, let's see where we get our radical drop!
...
```
In this case, two clusters seems optimal. We will therefore run `kmeans()`again with two clusters and visualize the results. I will split the pairs plot into two separate ones, for more readable results.
