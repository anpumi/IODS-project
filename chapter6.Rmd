...
# 6. Analysis of Longitudinal Data {.tabset .tabset-fade .tabset-pills}

>Welcome to the last of the weekly analysis entries! This week was interesting. Not only were there some major issues (more on those later), I also thought I'd experiment with the layout of the html some... As a result, please see the tabs for the two report sections (neat, huh?).
...
ggplot(RATSS, aes(x = Time, y = mean, linetype = Group, shape = Group), inherit.aes=FALSE) +
  geom_line() +
...
This obviously still leaves the outliers of the other two groups, and also makes the box for `Group 2` looking almost the same as the other two. The boxes being so short means that the standardized weights of the rats in each group are very close to one another.
...
The chi-squared statistic of the likelihood ratio test between `BPRS_ref` and `BPRS_ref` is within the 95% level of significance.
...
The second graph is interesting. It shows us the *fitted* values of the `bprs` scores.  