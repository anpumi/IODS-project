...
This code gives us both the `data.frame` with all the values from ten observations. R console gives us the 2x2 cross tabulation. 
...
The training error is approximately 0.25. 

Using the variables which, according to your logistic regression model, had a statistical relationship with high/low alcohol consumption, explore the predictive power of you model. DONE

Provide a 2x2 cross tabulation of predictions versus the actual values and optionally display a graphic visualizing both the actual values and the predictions. DONE

Compute the total proportion of inaccurately classified individuals (= the training error) DONE

and comment on all the results. 

Compare the performance of the model with performance achieved by some simple guessing strategy

### Part 7 - Bonus

As a bonus I will perform a 10-old cross-validation on my model. This is done with the so-called K-fold validation, where K is the number of folds.
...
My model has a very slightly better test set performance than the model that was introduced in DataCamp: 0.2617801 versus 0.2617801.

Now, the challenge is to find an even better model. Let's discard `famrel` from the model.

```{r}
model2 <- glm(high_use ~ sex + failures + absences, data = alc, family = "binomial")
loss_func <- function(class, prob) {
  n_wrong <- abs(class - prob) > 0.5
  mean(n_wrong)
}
...
By removing `famrel` from the model, we achieve a better test set performance than before at 0.2591623.
